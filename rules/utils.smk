...
rule remove_alt_chr_for_drop:
    input:
        out + "/{sample}/picard_markdupe/{sample}_{sample_number}_trim_star_marked.bam",
    output:
        proj_dir + "/drop_files/{sample}/{sample}_{sample_number}_trim_star_marked_no_alt.bam",
    params:
        job_name="{sample}_{sample_number}_chr_alt4removal",
    resources:
...
rule generate_drop_sample_files:
    input:
        ...
        fexist=expand([proj_dir + "/drop_files/{s.sample}/{s.sample}_{s.sample_number}_trim_star_marked_no_alt.bam"], s=samples.itertuples()),
    output:
